...
export default function Register() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // Redirect to dashboard or show check email depending on confirm settings
      navigate('/dashboard')
    } catch (err) {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-slate-950 overflow-hidden text-slate-100 font-sans py-12 px-4 sm:px-6 lg:px-8">
      ...
      {/* Glassmorphism Card */}
      <div className="relative max-w-md w-full space-y-8 bg-slate-900/50 backdrop-blur-2xl p-10 rounded-3xl shadow-2xl border border-slate-700/50 z-10 mx-auto">
        <div>
          ...
          <p className="mt-3 text-center text-sm text-slate-400">
            Or{' '}
            <Link to="/login" className="font-medium text-indigo-400 hover:text-indigo-300 transition-colors drop-shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                type="password"
                disabled={isSubmitting}
                className={`appearance-none block w-full px-4 py-3 bg-slate-950/50 border ${errors.confirmPassword ? 'border-red-500/50' : 'border-slate-700/50'} rounded-xl text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-transparent transition-all sm:text-sm`}
                placeholder="••••••••"
